...
будут работать точки останова (breakpoints).

В настоящий момент открывать проект в VS Code лучше
после установки NPM-пакетов, поскольку отключено
слежение за изменениями в папке "node_modules" со
стороны расширения Артель. Это также значит, что
при переустановке пакетов VS Code лучше перезапустить.
